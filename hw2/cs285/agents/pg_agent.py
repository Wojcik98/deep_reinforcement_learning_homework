...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [np.array(list(self._discounted_return(traj))) for traj in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # DONE: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [np.array(list(self._discounted_reward_to_go(traj))) for traj in rewards]
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            assert values.shape == q_values.shape, f"Shapes mismatch: {values.shape} vs {q_values.shape}"
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    advantages[i] = delta + self.gamma * self.gae_lambda * (1 - terminals[i]) * advantages[i + 1]
                    # advantages[i]= rewards[i] + self.gamma * ((1 - self.gae_lambda) * values[i + 1] + self.gae_lambda * advantages[i + 1])
        ...
        if self.normalize_advantages:
            print(f"Before: mean: {advantages.mean()}, std: {advantages.std()}")
            mean = np.mean(advantages)
            std = np.std(advantages)
            advantages = (advantages - mean) / (std + 1e-8)
            print(f"After: mean: {advantages.mean()}, std: {advantages.std()}")
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for reward in rewards:
            sum_ += discount * reward
            discount *= self.gamma

        return [sum_ for _ in rewards]
