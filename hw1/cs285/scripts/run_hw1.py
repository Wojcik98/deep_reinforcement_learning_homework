...
def run_training_loop(params):
    ...
    for itr in tqdm(range(params['n_iter'])):
        print("\n\n********** Iteration %i ************" % itr)
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, batch_size, batch_size)
            ...
            for path in paths:
                ob = path['observation']
                ac = expert_policy(ptu.from_numpy(ob))
                ac = ptu.to_numpy(ac)
                path['action'] = ac
            # paths = TODO
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
            ...
            indices = np.random.choice(len(replay_buffer), size=train_batch_size, replace=True)
